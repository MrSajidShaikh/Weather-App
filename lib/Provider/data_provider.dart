...
class DataProvider extends ChangeNotifier {
      ...
      if (await checkConnectivity()) {
        try {
          final x = await http.get(
              "http://dev.virtualearth.net/REST/v1/Locations/${_loc.latitude},${_loc.longitude}?includeEntityTypes=Postcode1&include={includeValue}&key=$locationSearchApi" as Uri);
          if (x.statusCode == 200) {
    ...
    if (await checkConnectivity()) {
      try {
        var x = await http.get(
            "http://dev.virtualearth.net/REST/v1/Locations/$inputText?o=json&maxResults=4&key=$locationSearchApi" as Uri);
  ...
  Future<int> _fetchWeather(City city, bool firstTime, bool temporary) async {
    ...
    try {
      final x = await http.get(
          "https://api.openweathermap.org/data/2.5/onecall?lat=${city.coordinates[0]}&lon=${city.coordinates[1]}&exclude=minutely&appid=$weatherApiKey" as Uri);
      if (x.statusCode == 200) {
        var json = jsonDecode(x.body);
